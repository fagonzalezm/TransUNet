...
from datasets.dataset_scian import Scian_dataset
...
parser.add_argument('--root_path', type=str,
                    default='../data/SCIAN-SpermSegGS-Original/head_npz', help='root dir for validation volume data')  # for acdc root_path=root_dir
...
def inference(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        metric_i = test_single_volume(image, label, model, classes=args.num_classes, patch_size=[args.img_size, args.img_size],
                                      test_save_path=test_save_path, case=case_name, z_spacing=args.z_spacing, need_zoom=1)
...
if __name__ == "__main__":
    ...
    writer.add_scalar('test/specificity', np.mean(metric_test, axis=0)[5], 0)
    writer.add_scalar('test/f1', np.mean(metric_test, axis=0)[6], 0)
    writer.add_scalar('test/accuracy', np.mean(metric_test, axis=0)[7], 0)
    writer.close()
